#!/usr/bin/env python3
"""
Script to fix broken headers on Spanish pages by copying the correct navigation
"""
...
# The correct Spanish navigation HTML with styles
CORRECT_SPANISH_NAV_HTML = '''    <!-- Navigation -->
    <nav>
        <div class="nav-container">
            ...
            <ul class="nav-links">
                ...
                <li><a href="/es/process/">Proceso</a></li>
...
# Navigation CSS styles (same as English)
...
def fix_spanish_file_header(file_path):
    """Fix the header in a single Spanish file"""
    try:
...
def find_all_spanish_html_files():
    """Find all Spanish HTML files that need fixing"""
    root_path = Path("/Users/glengomezmeade/plasma-pay-calculator")
    spanish_files = []
    
    # Get all HTML files in /es/ directories
    ...
    return spanish_files

def main():
    """Main function to fix all Spanish headers"""
    print("🔧 Fixing Spanish Page Headers...")
    print("=" * 50)
    
    spanish_files = find_all_spanish_html_files()
    print(f"Found {len(spanish_files)} Spanish HTML files to fix\n")
    ...
    print("📍 Processing priority Spanish files...")
    for filename in priority_files:
        for file_path in spanish_files:
            if filename in str(file_path):
                success, message = fix_spanish_file_header(file_path)
                if success:
                    fixed_count += 1
    ...
    print("\n🔄 Processing all other Spanish files...")
    processed_files = set()
    for file_path in spanish_files:
    ...
    print(f"\n✨ Spanish Header Fix Complete!")
    print(f"   Fixed: {fixed_count} files")
    print(f"   Errors: {error_count} files")
    print(f"   Total Spanish files: {len(spanish_files)}")
