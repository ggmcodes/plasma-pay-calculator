#!/usr/bin/env python3
"""
Script to standardize English page headers and mobile navigation across all calculator pages.
This script adds consistent header structure and mobile menu functionality.
"""
...
# Standard header HTML for English pages
...
def find_html_files():
    """Find all HTML files in calculator directories"""
    html_files = []
    
    # Get all calculator HTML files
    calculators_path = Path("/Users/glengomezmeade/plasma-pay-calculator/calculators")
    if calculators_path.exists():
        html_files.extend(list(calculators_path.rglob("*.html")))
    
    # Get blog index
    blog_index = Path("/Users/glengomezmeade/plasma-pay-calculator/blog/index.html")
    if blog_index.exists():
        html_files.append(blog_index)
    
    # Get centers and health pages
...
def is_spanish_file(file_path):
    """Check if file is in Spanish directory"""
    return "/es/" in str(file_path) or str(file_path).startswith("/Users/glengomezmeade/plasma-pay-calculator/es/")

def update_html_file(file_path):
    """Update a single HTML file with standardized header and mobile menu"""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Skip Spanish files
        if is_spanish_file(file_path):
            return False, "Skipped Spanish file"
